...
for line in sys.stdin:
    ...
    digits[9] = next(p for p in patterns if len(p) == 6 and digits[4] < p)
    digits[3] = next(p for p in patterns if len(p) == 5 and digits[7] < p)
    digits[0] = next(
        p
        for p in patterns
    ...
    digits[6] = next(
        p for p in patterns if len(p) == 6 and p != digits[0] and p != digits[9]
    )
    digits[5] = next(p for p in patterns if len(p) == 5 and p < digits[6])
    digits[2] = next(
        p for p in patterns if len(p) == 5 and p != digits[3] and p != digits[5]
    )
