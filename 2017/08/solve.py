...
Instruction = namedtuple('Instruction', ['register', 'op', 'value',
                                         'c_register', 'c_op', 'c_value'])
ops = {'inc': add, 'dec': sub, '!=': ne, '>=': ge, '<=': le, '>': gt, '<': lt,
       '==': eq}


def parse(line):
    tokens = line.strip().split(' ')
    return Instruction(tokens[0], ops[tokens[1]], int(tokens[2]), tokens[4],
                       ops[tokens[5]], int(tokens[6]))
...
def run(instructions):
    ...
    for instr in instructions:
        if instr.c_op(registers[instr.c_register], instr.c_value):
            registers[instr.register] = instr.op(registers[instr.register],
                                                 instr.value)
